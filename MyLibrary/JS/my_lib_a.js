 ...
 * @returns {undefined}
 */
function insertHTMLIntoAnyElement(element_id, html_code) {
    //Example:  $("#aa").html("<p>test paragraph</p>");
...
function date_get_month_corrected(month) {
    if (month < 10) {
        return "0" + month;
    } else {
        return month;
...
function date_get_month_name_eng(month) {
...
function date_get_day_corrected(day) {
    if (day < 10) {
        return "0" + day;
    } else {
        return day;
...
function date_validate_format_mm_dd_yyyy(subject) {
...
function date_validate_format_yyyy_mm_dd(dateString) {
 ...
 * @tags ajax, xmlhttp
 */
function ajaxRequest_old(recieving_script, paramter_name, value_to_send, asynchron) {
    var xmlhttp;
 ...
 * @returns {undefined}
 */
function animation_fade_in(element_id_class_tag) {
 ...
 * @returns {undefined}
 */
function animation_slide_down(element_id_class_tag) {
 ...
 * @returns {undefined}
 */
function animation_flash_example() {
    //This one is to start the "Thread"; 
    var intervalID = setInterval(blinker, 1000);
    //This one is to stop the "Thread"
    window.clearInterval(intervalID);
}
function blinker() {
    if (isVisible("loading_notification_text")) {
        $('#elem_div_class..').fadeOut(1000);
        $('#elem_div_class').fadeIn(1000);
    }
}
//============================================
...
/**
 * Listing all available attributes of an Element
 * @tags list attribute, show all attributes, atribute
 * @param {Element} element
 * @returns {String}
 */
function listAttributesOfElement(element) {
    for (var propertyName in element) {
        console.log("attribute = " + propertyName);
    }
}
...
/**
 * Dont use "contains" method ex string.contains("$")
 * contains method works only with firefox
 * @param {type} string
 * @param {type} searched_string
 * @returns {Boolean}
 */
