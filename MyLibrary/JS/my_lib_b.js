...
/**
 * Adds text to the given parent. The element which 
 * includes the text can be adjusted, you can set elem "p" or "h1" or ...
 *<ul> 
 *<li>$text = the text to add
 *<li>$parent = the element to with the child ("p" or "h1", is going to be added to)
 *<li>$newLineTag = the element which is the parent for the text element without <> !!!
 *</ul>
 */
function writeLineAdjusted(text, parent, newLineTag) {

    //get the element to add to
    var elemToInsertTo = document.getElementById(parent);

    //Create the texnode from the in param text
    var textToInsert = document.createTextNode(text);

    //Create new element
    var paragraphH1 = document.createElement(newLineTag);

    //Add child to the h1 element in form of the textElement
    paragraphH1.appendChild(textToInsert);

    //Insert the h1 element to the "mainContainer"
    elemToInsertTo.appendChild(paragraphH1);

}

/**
 *<p>Adds a footer with elements which acts like buttons:
 * <div display="inline"><strong>Kontak  |  Info  |  L�nkar </strong></div> 
 *
 *<p>It may also require a CSS entry to mark and underline the foot element like:
 *
 *<ul>
 *<li>div.one:hover { 
 *<li>text-decoration: underline;
 *<li>background-color:#B0B0B0;
 *<li>cursor: crosshair;
 *<li>}
 *</ul>
 *
 ...
 * <p>methods[1] = test2;
 * <p>methods[2] = test3;
 * <p><strong>addFooter("foot", arr,methods);</strong></p>
 * 
 * <ul>
 *  <li>$parent = the element to add to
 *  <li>$footElementArr = Array that contains titles for the footer element
 *  <li>$methodsArr = The array containing names of the Methods which are run by clicking an element,
 *   note those are not provided as String but like - $method1$ without () or ""
 *  </ul>
 */
